...
def main():
    ...
    # --- Stage 1 (optional — skipped if no emails/ dir or no .msg files) ---
    emails_data = []
    msg_files   = list(emails_dir.glob("*.msg")) if emails_dir.exists() else []

    if not emails_dir.exists():
        print("\n⚠  Stage 1: emails/ directory not found — skipping .msg conversion.")
    elif not msg_files:
        print("\n⚠  Stage 1: No .msg files found in emails/ — skipping conversion.")
    else:
        print(f"\n🔄 Stage 1: Converting {len(msg_files)} .msg file(s) to markdown ...")
        for msg_file in msg_files:
            result = convert_msg_to_md(msg_file, markdown_dir)
            if result:
                emails_data.append(result)

    # Check there is something to work with before continuing
    existing_md = [
        f for f in markdown_dir.glob("*.md")
        if not f.name.startswith("timeline_") and not f.name.startswith("FML-CHECKLIST")
    ]
    if not emails_data and not existing_md:
        print("✗ No emails processed and no existing markdown files found — nothing to do.")
        return

    # --- Stage 2 (only if Stage 1 produced new data) ---
    if emails_data:
        print("\n📅 Stage 2: Generating timeline ...")
        generate_timeline_summary(emails_data, timeline_path)
    else:
        print("\n⏭  Stage 2: Skipped (no new emails — using existing markdown).")
